...
begin
  ...
  Formula := BlackForest_u.TFormula.Create(1);
  MenuNames[0]          := mnmns1 ;
  ...
  ShowMessage('1 ');
  Formula.RetrieveDrinkNames;//=
  ShowMessage('2 ');
  Formula.CalculateDrinkPrices;//=
  ShowMessage('3 ');
  Formula.RetrieveRecipeIDs;//=
  ShowMessage('4 ');
  Formula.RetrieveRecipeNames;//=
  ShowMessage('5 ');
  Formula.RetrieveRecipeServings;//=
  ShowMessage('6 ');
  Formula.CalculateRecipePrices;//=
  ShowMessage('7 ');
  FillMenuInformation(MenuNames[1]);
  CheckStock(MenuNames[1]);
  FillMenuInformation(MenuNames[2]);
  CheckStock(MenuNames[2]);
  FillMenuInformation(MenuNames[3]);
  CheckStock(MenuNames[3]);
  FillMenuInformation(MenuNames[4]);
  CheckStock(MenuNames[4]);
...
begin
    ...
    if (lgnfrm1.edtUsername.Text ='') or (lgnfrm1.edtPassword.Text ='') then
    begin
    ...
    begin
      ...
      begin
        ...
        Parameters.ParamByName('Username').Value := lgnfrm1.edtPassword.Text;
        Open;
        ...
        begin
          if FieldByName('Password').AsString <> lgnfrm1.edtpassword.Text then
          Begin
            Application.MessageBox('Incorrect Login Credentials!', 'Error', MB_OK +
            MB_ICONSTOP + MB_DEFBUTTON2);
          End
          ...
          begin
            ...
            lgnfrm1.edtUsername.Clear;
            lgnfrm1.edtPassword.Clear;
...
begin
  Formula.ShowLedger(ldgr1.dbgrdLedger);
  admnbtns1.Visible := False;
  ldgr1.Visible := True;
end;
...
begin
  ...
  begin
    try
      ...
      Formula.CheckOut(CustomerTotal,TableNumber);
    except
...
begin
  ...
  begin
    ...
    begin
      for I := 1 to 14 do
      begin
        if not Formula.CheckStock(Counter,I-1) then
        begin
  ...
  begin
    ...
    begin
      with MenuName do
      begin
        if not Formula.CheckDrinks(I-1) then
        begin
          Button := FindComponent('btn' + IntToStr(i)) as TButton;
          Button.Visible := False;
        end
